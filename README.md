...
一组可独立安装、默认离线、结果可追溯的审计 Agent Skills。第一版包含：

- `expense-audit`：费用、报销与发票异常全量扫描。
- `procurement-fraud`：采购舞弊红旗、供应商关系、价格、拆单、流程和标书相似度筛查。
- `investigation-assistant`：把举报、邮件、消息和日志整理成证据清单、时间线与调查工作空间。
...
python3 skills/expense-audit/scripts/run_expense_audit.py \
  --input skills/expense-audit/examples/input/expenses.csv \
  --policy skills/expense-audit/examples/input/policy.json \
  --output /tmp/expense-audit-demo

python3 skills/procurement-fraud/scripts/run_procurement_audit.py \
  --input-dir skills/procurement-fraud/examples/input \
  --config skills/procurement-fraud/examples/input/config.json \
  --output /tmp/procurement-audit-demo

python3 skills/investigation-assistant/scripts/build_case_workspace.py \
  --input-dir skills/investigation-assistant/examples/input \
  --scope skills/investigation-assistant/examples/input/scope.json \
...
```bash
python3 -m unittest discover -s skills/expense-audit/tests -v
python3 -m unittest discover -s skills/procurement-fraud/tests -v
python3 -m unittest discover -s skills/investigation-assistant/tests -v
...
每个 `skills/<name>/` 文件夹都可以单独复制。各宿主的放置位置见 `adapters/`。Canonical skill 不依赖任何宿主私有字段；`agents/openai.yaml` 只提供 Codex/ChatGPT 的可选界面信息。

`dist/` 提供按 skill 分开的 ZIP 快照。正式使用前仍应阅读 `SKILL.md`、检查脚本，并先运行包内合成样例。路由验收必须在独立的新会话中执行，不能在已透露预期 skill 的同一上下文里自测。
...
└── evals/                  pack 级结构、触发和跨 Agent 评估
```

当前推荐版本：`expense-audit 0.1.1`、`procurement-fraud 0.1.1`、`investigation-assistant 0.1.2`。
