...
def process_messages(
    ...
    normalized, excluded, warnings, corpus = [], [], [], []
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row_number, source in enumerate(reader, start=2):
            ...
            if parsed is None:
                excluded.append({"source_file": relative, "source_row": row_number, "reason": warning or "时间戳无效", "evidence_id": evidence_id})
                continue
            allowed, reason = row_in_scope(parsed, (row["sender"], row["recipient"]), scope)
            if not allowed:
                excluded.append({"source_file": relative, "source_row": row_number, "reason": reason, "evidence_id": evidence_id})
                continue
            item = dict(row)
    ...
    return normalized, excluded, warnings, corpus
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.check_env:
        print(json.dumps({"python": sys.version.split()[0], "recommended": ">=3.10", "formats": ["csv", "txt", "md"], "network_required": False}, ensure_ascii=False))
        return 0
    ...
    messages, logs, text_lines, excluded, warnings, corpus = [], [], [], [], [], []
    for relative, source in allowed:
        kind = source_type(relative, scope)
        if kind == "messages":
            rows, out, notes, entries = process_messages(source, relative, file_hashes[relative], scope, registry)
            messages.extend(rows); excluded.extend(out); warnings.extend(notes); corpus.extend(entries)
        elif kind == "logs":
            rows, out, notes, entries = process_logs(source, relative, file_hashes[relative], scope, registry)
            logs.extend(rows); excluded.extend(out); warnings.extend(notes); corpus.extend(entries)
        elif kind == "text":
    ...
    write_csv(output / "out_of_scope_rows.csv", excluded, ("source_file", "source_row", "reason", "evidence_id"))

    timeline = []
    for message in messages:
        timeline.append({"timestamp": message["timestamp"], "actor": message["sender"], "event_type": "message", "object": "%s: %s" % (message["recipient"], message["content"][:120]), "source_file": message["source_file"], "source_row": message["source_row"], "evidence_id": message["evidence_id"], "confidence": 1.0})
    for log in logs:
    ...
    quality = [
        ...
        "- 文本证据行：%d" % len(text_lines), "- 范围外或时间无效行：%d" % len(excluded),
    ...
    manifest = {
        ...
        "warnings": warnings, "network_access": False,
        "outputs": ["evidence/raw/", "derived/", "evidence_inventory.jsonl", "evidence.jsonl", "chain_of_custody.jsonl", "out_of_scope_rows.csv", "timeline.csv", "entity_index.csv", "relationships.json", "evidence_matrix.csv", "hypothesis_register.csv", "findings.jsonl", "interview_plan.csv", "case_memo_template.md", "data_quality.md", "run_manifest.json"],
    }
    (output / "run_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({"output": str(output), "raw_files": len(inventory), "timeline_rows": len(timeline), "out_of_scope_rows": len(excluded), "issues": len(matrix), "findings": len(findings)}, ensure_ascii=False))
    return 0
