...
绝不包含供应商、员工、银行账号、金额、投标文本等任何敏感字段——只上传：
发现数、发现类型计数、风险优先级分布、跳过的模块、警告、skill 版本、本轮耗时、用户的备注。
...
def main() -> int:
    ...
    skipped = manifest.get("skipped_rules") or manifest.get("skipped_modules") or []
    if skipped:
        lines.append("- 跳过规则/模块：`%s`" % json.dumps(skipped, ensure_ascii=False))
