...
╚══════════════════════════════════════════════════════════════════════════════╝
   ...
   │     Step 3: 写 state.json                                        │
   │     Step 4: 调 MiniMax API，写 proposals/                         │
   │                                                                   │
   └─────────────────────┬───────────────────────────────────────────┘
                         │
                    【HTTP API】
...
╚══════════════════════════════════════════════════════════════════════════════╝
  ...
  │  ✅ API key 验证      sk-cp--... 通过, 模型返回正确             │
  │  ✅ 端到端跑通       test → score → LLM → proposal 全跑通        │
  │  ✅ 第一个 proposal   5893 字符 LLM 分析已生成                  │
  └──────────────────────────────────────────────────────────────────┘

  ┌─ 待做 (❌) ────────────────────────────────────────────────────┐
  │  ❌ 通知机制        邮件通知（让用户知道系统在跑什么）           │
  │  ❌ launchd 部署    在 Mac mini 上跑（24h 自动化）               │
  │  ❌ 健康检查       token 耗尽 / API 挂了的告警                  │
